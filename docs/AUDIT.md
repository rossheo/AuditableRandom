...
| 추출 종류와 인자 | 어떤 메서드를 어떤 범위로 호출했는지 (예: `Next(userId, 1000)`). 값 유도에 필요하다. |
...
| 메서드 | 워드 읽기 | 유도 |
|---|---|---|
| `NextUInt32(userId, max)` / `Next(userId, max)` | 블록을 앞에서부터 4바이트 **big-endian** 워드 16개로 스캔 | 첫 번째 수락 워드에 Lemire(N=32, range=max) 적용 |
...
| `Hits(userId, numerator, denominator)` | `Next(userId, denominator)`와 동일 | 뽑힌 값이 `numerator` 미만이면 명중(`true`). 명중 확률은 정확히 `numerator/denominator` |
...
```csharp
// Next(userId, max)로 뽑힌 값을 (seed, userId, tick)에서 재현한다.
